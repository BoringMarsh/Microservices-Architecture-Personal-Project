<template>
    <div v-if="showRouteFromView" class="city-search-result">
        <div v-if="resultCities.length != 0" class="success-header">
            <h2 style="color: #196883;"> 仅需一步即可全面了解城市！ </h2>
            <p> 告诉我们你想去哪座城市！ </p>
            ...
            <div class="table-container">
                <el-table :data="resultCities" class="table" :header-cell-style="{
                    ...
                    <el-table-column label="位置" align="center" width="300">
                        <template #default="scope">
                            <label class="row-path"> {{ scope.row.path }} </label>
                        </template>
                    </el-table-column>
                    <el-table-column label="时区" align="center">
                        <template #default="scope">
                            <label class="row-timezone"> {{ scope.row.timezone + "(" + scope.row.time_converter + ")" }} </label>
                        </template>
                    </el-table-column>
